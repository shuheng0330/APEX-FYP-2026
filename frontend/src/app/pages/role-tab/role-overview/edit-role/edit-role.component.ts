...
@Component({
  ...
  imports: [CommonModule, NzButtonModule, NzDrawerModule, NzFormModule, NzInputModule, NzSelectModule,
    TranslateModule, NzIconModule, FormsModule, ReactiveFormsModule, NzModalModule, NzAutocompleteModule],
...
export class EditRoleComponent {
  ...
  private readonly confirmTitleKey = "PAGE.ROLE.OVERVIEW.EDIT.CONFIRM.TITLE";
  private readonly confirmContentKey = "PAGE.ROLE.OVERVIEW.EDIT.CONFIRM.CONTENT";
  ...
  initialize(row: RoleOverview): void {
    ...
    this.selectedRecord!.assignedJobScopes?.forEach((jobScope) => {
      this.listOfControl.push(this.fb.control(jobScope.jobScope));
    });
  ...
  showConfirm(): void {
    if (this.validateForm.valid) {
      ...
      const orgChart = this.orgChartList.find(orgChart => orgChart.id === departmentId);

      const roleConfirm = `<li>${roleName} (${orgChart?.name})</li>`
  ...
  submit(): void {
    if (this.validateForm.valid) {
      ...
      const roleId = this.selectedRecord?.roleId!;
      const roleName = this.validateForm.get('role')?.value;
      ...
      this.roleService.updateRole(requestBody).subscribe({
        next: (res) => {
          this.formSubmitted.emit();
          this.updatedRole.emit(res.updatedRole);
